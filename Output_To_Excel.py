...
def user_video_list(username, video_count=1):
    ...
    # verify key
    verifyFp = "verify_km0n7n47_NhKUbiyZ_A4Gq_4PRb_A5ho_J7oNkT8Tpjrj"
...
def all_users_data():
    ...
    for user in users_array:
        videos_data = user_video_list(user, video_count=3)
...
def yahoo_finance_search(words):
    ...
    if soup.findAll(class_='D(ib) Fz(18px)'):
        ...
        try:
            ...
            return found_possibilities + 'Word Searched: ' + words
        except:
            print('Error')
    else:
        print('No data')
...
def video_data(videos_array):
    ...
    for i in range(len(videos_array)):
        ...
        for i in range(len(words)):
            ...
            sorted_common_words = sorted(Common_Words.common_words)

            lines = str(words[i])
            for word in lines.split():
                if word.lower() not in sorted_common_words:
    ...
    with open('output_excel_' + str(date.today()) + '.csv', 'w') as outfile:
        writer = DictWriter(outfile, ('stock_name', 'user_name', 'video_id', 'followers', 'views', 'likes', 'comments', 'shares'))
        writer.writeheader()
